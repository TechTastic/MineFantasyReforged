...
public class CustomCrafterEntry {
    ...
    public static void registerItem(ItemStack piece, String type, float efficiency, int tier) {
        entries.put(BuiltInRegistries.ITEM.getKey(piece.getItem()), new CustomCrafterEntry(piece.getItem(), type, efficiency, tier));
    }
    ...
    public static CustomCrafterEntry getEntry(ItemStack piece) {
        if (piece != null) {
            if (entries.containsKey(BuiltInRegistries.ITEM.getKey(piece.getItem())))
                return entries.get(BuiltInRegistries.ITEM.getKey(piece.getItem()));
        }
        return null;
    }
}
